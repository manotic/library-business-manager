<div class="space-y-6" x-data="{ privacy: @entangle('showData'), notification: '', showNotify: false }" 
    ...
    <div x-show="showNotify" x-transition class="fixed bottom-10 left-1/2 -translate-x-1/2 z-[100] bg-red-600 text-white px-8 py-4 rounded-full shadow-2xl flex items-center gap-3">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="bg-gray-800 border border-gray-700 p-4 rounded-2xl border-l-4 border-l-red-500">
            ...
            <p class="text-xl font-black text-white" :class="!privacy && 'blur-md'">{{ number_format($stats['total_spent']) }}</p>
        </div>
    ...
    <div class="bg-gray-800 p-6 rounded-xl border border-gray-700 space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <input type="text" wire:model.live="search" placeholder="Search description..." class="p-3 bg-gray-900 border-gray-700 rounded-lg text-sm text-white focus:ring-red-500">
            ...
            <select wire:model.live="filter_category" class="p-3 bg-gray-900 border-gray-700 rounded-lg text-sm text-white">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div class="lg:col-span-4 bg-gray-800 border border-gray-700 rounded-xl p-6 h-fit">
            ...
            <form wire:submit.prevent="save" class="space-y-4">
                ...
                <input type="date" wire:model="date" class="p-3 w-full bg-gray-900 border-gray-700 rounded-lg text-white">
                ...
                <button type="submit" class="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-4 rounded-xl shadow-lg transition-transform active:scale-95 uppercase tracking-widest">Record Expense</button>
            </form>
        </div>
                ...
                <table class="w-full text-left text-sm">
                    ...
                    <tbody class="divide-y divide-gray-700">
                        ...
                        <tr class="hover:bg-gray-700/30 transition-colors">
                            ...
                            <td class="px-4 py-4 font-bold text-red-400 uppercase text-[10px]">{{ $record->category }}</td>
                            <td class="px-4 py-4 text-white font-bold" :class="!privacy && 'blur-md'">
                                - {{ number_format($record->amount) }}
                            </td>
                            <td class="px-4 py-4 text-gray-400 italic text-[11px]">{{ $record->description ?? 'No details' }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
