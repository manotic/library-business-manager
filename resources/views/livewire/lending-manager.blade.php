<div class="space-y-6" x-data="{ 
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
        <div class="bg-gray-800 border border-gray-700 p-4 rounded-2xl">
            <p class="text-gray-400 text-[10px] font-bold uppercase">Total Lent</p>
        ...
        <div class="bg-gray-800 border border-gray-700 p-4 rounded-2xl border-l-4 border-l-green-500">
            <p class="text-gray-400 text-[10px] font-bold uppercase text-green-400">Net Return</p>
            <p class="text-xl font-black text-green-400" :class="!privacy && 'blur-md'">{{ number_format($stats['actual_collected']) }}</p>
        </div>
        ...
        <div class="bg-gray-800 border border-gray-700 p-4 rounded-2xl">
            <p class="text-gray-400 text-[10px] font-bold uppercase">Target</p>
            <p class="text-xl font-black text-indigo-400" :class="!privacy && 'blur-md'">{{ number_format($stats['total_invested']) }}</p>
        </div>
    </div>

    <div class="bg-gray-800 p-6 rounded-xl border border-gray-700 space-y-4">
        <div class="flex justify-between items-center">
            <h2 class="text-xl font-bold text-white italic">Lending Records</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <input type="text" wire:model.live="search" placeholder="Search source or name..." class="p-3 bg-gray-900 border-gray-700 rounded-lg text-sm text-white focus:ring-indigo-500">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div class="lg:col-span-4 bg-gray-800 border border-gray-700 rounded-xl p-6 h-fit">
            <h3 class="text-white font-bold mb-4 flex items-center gap-2">
                <i data-lucide="plus-circle" class="w-5 h-5 text-indigo-400"></i> New Entry
            </h3>
            <form wire:submit.prevent="save" class="space-y-4">
                <input type="text" wire:model="source" placeholder="Source / Person Name" class="p-3 w-full bg-gray-900 border-gray-700 rounded-lg text-white">
                ...
                <input type="number" wire:model="amount" placeholder="Principal Amount" class="p-3 w-full bg-gray-900 border-gray-700 rounded-lg text-white">
                <textarea wire:model="description" placeholder="Description (Optional)" class="p-3 w-full bg-gray-900 border-gray-700 rounded-lg text-white h-20"></textarea>
                <button type="submit" class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-4 rounded-xl shadow-lg transition-transform active:scale-95 uppercase tracking-wider">Record Money Lent</button>
            </form>
        </div>
                ...
                <table class="w-full text-left text-sm">
                    <thead class="bg-gray-700/50 text-gray-300 uppercase text-[10px]">
                        <tr>
                            ...
                            <th class="px-4 py-4">Source/Contact</th>
                    ...
                    <tbody class="divide-y divide-gray-700">
                        ...
                        @php $balance = $record->amount - $record->amount_returned; @endphp
                        <tr class="hover:bg-gray-700/30 transition-colors">
                            ...
                            <td class="px-4 py-4">
                                ...
                                <p class="text-[10px] text-gray-500">{{ $record->phone ?? 'No Contact' }}</p>
                            </td>
                            <td class="px-4 py-4" :class="!privacy && 'blur-md'">
                                <p class="text-white font-bold">{{ number_format($record->amount) }}</p>
                                <p class="text-[10px] text-gray-500">Ret: {{ number_format($record->amount_returned) }}</p>
                            </td>
                            <td class="px-4 py-4">
                                @if($balance <= 0)
                                    <span class="px-2 py-1 bg-green-500/10 text-green-500 text-[10px] font-bold rounded">FULLY PAID</span>
                                @else
                                    <p class="text-red-400 font-bold" :class="!privacy && 'blur-md'">{{ number_format($balance) }}</p>
                                    <div class="w-full bg-gray-900 h-1 rounded-full mt-1">
                                        <div class="bg-indigo-500 h-1 rounded-full" style="width: {{ ($record->amount_returned / $record->amount) * 100 }}%"></div>
                                    </div>
                                @endif
                            </td>
                            <td class="px-4 py-4 text-center">
                                @if($balance > 0)
                                    <div class="flex gap-2 justify-center">
                                        <input type="number" wire:model="additional_payment.{{ $record->id }}" placeholder="+" class="w-16 p-1 bg-gray-900 border-gray-700 rounded text-xs text-white">
